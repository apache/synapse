...
public class ResequenceMessageStore extends JDBCMessageStore {
    ...
    private void initResequenceParams(Map<String, Object> parameters) {
        xPath = (SynapseXPath) parameters.get(ResequenceMessageStoreConstants.SEQUENCE_NUMBER_XPATH);
        ...
        if (gapTimeoutInterval >= 0) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Resequencer initialized with xpath:" + xPath.toString() + ",the waiting count configured:"
                        + gapTimeoutInterval);
            }
        } else {
            nextElapsedTime = -1;
